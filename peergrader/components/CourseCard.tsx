...
export default function CourseCard({ courseData }: { courseData: CourseData }) {
    return (<div className="rounded-lg overflow-hidden shadow-lg border">
        <Link href={`/courses/${courseData.course_id}`} className="block">
            <div className="p-4">
                <div className="text-lg font-bold truncate">{courseData.number ?? courseData.name}</div>
                <hr className="my-1 border-t-2"></hr>
                <div className="text-md text-gray-600 truncate mt-4">{courseData.number == null ? String.fromCharCode(160) : (courseData.name != null ? courseData.name: String.fromCharCode(160))}</div>
            </div>

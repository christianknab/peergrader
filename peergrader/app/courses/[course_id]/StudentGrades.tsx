...
export default function StudentGrades({ showGrades, onClose, uid, course_id }: StudentGradesProps) {
    ...
    useEffect(() => {
        if (showGrades) { fetchAsgns().then(setAsgns); }
    }, [uid, course_id]);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-4 relative w-3/4 max-w-4xl">
                <button className="absolute top-0 right-0 m-4 write-grey hover:text-gray-700" onClick={onClose}>
                        ...
                        <table className="w-full border-collapse bg-white">
                            <thead>
                                <tr className="bg-gray-200">
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium write-grey uppercase tracking-wider">Phase</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium write-grey uppercase tracking-wider">Final Score</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-gray-200">
                                {asgns.map((asgn) => (
                                    <tr key={asgn?.asgn_id} className="hover:bg-gray-100">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{asgn?.final_score}/{asgn.max_score}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>

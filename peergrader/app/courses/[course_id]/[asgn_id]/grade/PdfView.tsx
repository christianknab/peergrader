                    ...
                    (_, index) => (
                        <div key={`page-container-extern${index + 1}`} className="flex justify-center mb-2 mt-2">
                            <div key={`page-container-${index + 1}`} className={`${pointSelectionEnabled && "cursor-pointer"} relative`} >
                                ...
                                />
                                {annotationMarkers.map((value, markIndex) => ((value.page == index + 1 && markIndex != excludeIndex) && <Marker selected={selectedIndex == markIndex} index={markIndex} radius={width ? width * 0.016 : 10} data={value} page={pageLayouts[index]} key={`marker${markIndex}`} />))}

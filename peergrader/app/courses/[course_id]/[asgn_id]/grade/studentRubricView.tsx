...
export const StudentRubric = ({ setSelectedPoints, selectedPoints, setPointsGiven, pointsGiven, rubric, maxPoints, setTotal, total, numInput }: StudentRubricProps) => {
                    ...
                    <table className="w-full table-auto border-collapse">
                        <thead>
                            <tr className="bg-gray-100">
                                {!numInput && <th className="border p-2 w-1/6">{selectedPoints[index] != -1 ? rubricItem.col_points[selectedPoints[index]] : 0}/{rubricItem.row_points[0]}</th>}
                                {numInput && <th className="border p-2 w-1/6">{Number.isNaN(pointsGiven[index]) ? 0 : pointsGiven[index]}/{rubricItem.row_points[0]}</th>}
                            ...
                            {rubricItem.descriptions.map((description, descIndex) => (
                                <tr key={descIndex}>
                                    <td className="border-l border-b p-2 w-1/6">
                                        ...
                                        {numInput && <input type="number" className={`[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none border text-sm rounded-lg block w-full p-2.5 ${((pointsGiven[index] > rubric[index].row_points[0]) || (Number.isNaN(pointsGiven[index]))) ? 'bg-red-200' : ''}`} placeholder="" value={pointsGiven[index]} required onChange={(e) => handleRowPointChange(index, e.target.valueAsNumber)} title={((pointsGiven[index] > rubric[index].row_points[0]) || (Number.isNaN(pointsGiven[index]))) ? 'Must enter a valid number' : ''} />}
                                    </td>
